...
function ensuredb(){
 if (!indexdb){
   indexdb = new Dexie("transaction");
   indexdb.version(1).stores({
     transactions: "++id, name, value, date, remoteID",
...
function saveRecord(transaction){
 ...
 if (typeof transaction._id !== "undefined"){
  transaction.remoteID = transaction._id;
  delete transaction._id;
 }
 
 indexdb.transactions.add(transaction)
} 
...
window.addEventListener("online", function(event){
  console.log(event);
  ensuredb();
  let unsaved = indexdb.transactions.each(transaction => {
    sendToserver(transaction);
  });
